  ...
  return (
    <div className="w-full max-w-5xl mx-auto font-sans bg-[#f8fafc] text-slate-900 rounded-sm shadow-2xl overflow-hidden print:bg-white print:shadow-none print:w-full">
      ...
      <div className="p-8 md:p-12 pb-6 border-b-2 border-slate-800">
        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
          <div className="flex-1">
            ...
            <p className="text-sm font-semibold text-slate-600 mt-2 uppercase tracking-widest">Internal Case: {report.reportId}</p>
            <p className="text-xs text-slate-500 font-mono mt-1">Tenant Authority: {report.tenantDomain}</p>
            ...
            <div className="mt-6 inline-flex items-start gap-2 p-3 border-2 border-emerald-700/50 rounded text-emerald-800 bg-emerald-50/50">
              ...
              <div>
                ...
                <p className="text-[10px] font-mono opacity-80">{report.cryptographicIntegrityHash}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-3 w-full md:w-auto">
            ...
            <div className="text-right mt-2">
              <p className="text-xs text-slate-500 font-mono">Date Generated: {new Date(report.generatedAtISO).toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-8 md:p-12 pt-8 flex flex-col gap-12">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              ...
              <span className="text-sm font-medium text-slate-900">{report.primaryAllegedOffender}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-slate-900">
                {report.totalEvidenceCount} items from {report.identifiedPlatforms.join(', ')}
              </span>
            </div>
          </div>
        </section>
        ...
        {/* LEGAL SECTIONS */}
        {report.applicableLegalSections.length > 0 && (
          <section>
        ...
        <section className="print:break-before-page">
          ...
          <div className="bg-slate-900 rounded-xl p-6">
            ...
            <TimelineViewer items={report.chronologicalTimeline} />
          </div>
        </section>
